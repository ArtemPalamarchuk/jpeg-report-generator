              ...
              <div className="space-y-4 pt-4 border-t border-gray-200">
                <div className="bg-green-50 border border-green-200 rounded-md p-4">
                  <p className="text-sm text-green-800">
                    ✅ Loaded: {reportData.token} • {reportData.exchanges.length} exchanges • {reportData.balances.length} balances
                  </p>
                </div>
                      ...
                      <div className="overflow-x-auto border border-gray-200 rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                          ...
                          <tr>
                            ...
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Notional</th>
                          </tr>
                          </thead>
                          <tbody className="bg-white divide-y divide-gray-200">
                          {reportData.balances.map((balance, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="px-3 py-2 text-sm font-medium text-gray-900">{balance.asset}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${balance.price.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">{balance.amount.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${balance.notional.toLocaleString()}</td>
                              </tr>
                          ))}
                          </tbody>
                        </table>
                    ...
                    <div className="pt-4">
                      ...
                      <div className="overflow-x-auto border border-gray-200 rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                          <tr>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Exchange</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Symbol</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">2% Liq</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">JPEG 2% Liq</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Liq Share</th>
                          </tr>
                          </thead>
                          <tbody className="bg-white divide-y divide-gray-200">
                          {reportData.exchanges.map((exchange, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="px-3 py-2 text-sm font-medium text-gray-900">{exchange.venue}</td>
                                <td className="px-3 py-2 text-sm text-gray-600">{exchange.symbol}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${exchange.liquidity2pct.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${exchange.jpegLiquidity2pct.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-600">{exchange.liquidityShare.toFixed(2)}%</td>
                              </tr>
                          ))}
                          </tbody>
                        </table>
                      ...
                      <div className="overflow-x-auto border border-gray-200 rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                          ...
                          <tr>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Exchange</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Symbol</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">JPEG Volume</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Market Volume</th>
                            <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Market Share</th>
                          </tr>
                          </thead>
                          <tbody className="bg-white divide-y divide-gray-200">
                          {reportData.exchanges.map((exchange, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="px-3 py-2 text-sm font-medium text-gray-900">{exchange.venue}</td>
                                <td className="px-3 py-2 text-sm text-gray-600">{exchange.symbol}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${exchange.jpegVolume.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-900">${exchange.marketVolume.toLocaleString()}</td>
                                <td className="px-3 py-2 text-sm text-right text-gray-600">{exchange.marketShare.toFixed(2)}%</td>
                              </tr>
                          ))}
                          </tbody>
                        </table>
